...
// safeLog маскирует токен в логах
func safeLog(level, format string, v ...interface{}) {
	msg := fmt.Sprintf(format, v...)
	if cfg.TgToken != "" {
		msg = strings.ReplaceAll(msg, cfg.TgToken, "*************/")
	}
...
func main() {
	...
	if err != nil {
		cleanErr := strings.ReplaceAll(err.Error(), cfg.TgToken, "*************/")
	...
	for update := range updates {
		if update.Message == nil || update.Message.From.ID != cfg.MyTelegramID {
			continue
		}
		msg := update.Message
...
func sendStatus(bot *tgbotapi.BotAPI, chatID int64) {
	...
	edit := tgbotapi.NewEditMessageText(chatID, m.MessageID, res)
	edit.ParseMode = "HTML"
	bot.Send(edit)
}
...
func handleTgToEmail(bot *tgbotapi.BotAPI, msgs []*tgbotapi.Message) {
	...
	if bodyText == "" && mainMsg.Document == nil && len(mainMsg.Photo) == 0 {
		return
	}
	...
	if mainMsg.ReplyToMessage != nil {
		...
		if orig == "" {
			orig = mainMsg.ReplyToMessage.Caption
		}
	...
	if targetEmail == "" {
		...
		if len(pts) == 2 && strings.Contains(pts[0], "@") {
			...
			bot.Send(tgbotapi.NewMessage(mainMsg.Chat.ID, "⚠️ Используйте Reply или 'адрес: текст'"))
			return
		}
	}

	if targetSub != "" && !strings.HasPrefix(strings.ToLower(targetSub), "re:") {
		targetSub = "Re: " + targetSub
	}
	...
	for _, m := range msgs {
		...
		if m.Document != nil {
			fID, fName = m.Document.FileID, m.Document.FileName
		} else if len(m.Photo) > 0 {
			fID, fName = m.Photo[len(m.Photo)-1].FileID, fmt.Sprintf("img_%d.jpg", time.Now().Unix())
		}
		if fID != "" {
...
func tryAppendSent(e *email.Email) {
	...
	for _, f := range commonSentFolders {
		if err := c.Append(f, []string{imap.SeenFlag}, time.Now(), bytes.NewReader(raw)); err == nil {
			break
		}
	}
}
...
func emailToTgLoop(bot *tgbotapi.BotAPI) {
	for {
		if err := pollEmails(bot); err != nil {
			safeLog("ERROR", "IMAP: %v. Повтор через %d сек...", err, cfg.RetryDelaySec)
...
func processEmailToTg(m *imap.Message, bot *tgbotapi.BotAPI, c *client.Client, seq *imap.SeqSet) {
	...
	for {
		...
		if strings.Contains(strings.ToLower(disp), "attachment") {
			...
			body = string(b)
		} else if strings.HasPrefix(ct, "text/html") && body == "" {
			body = html2text.HTML2Text(string(b))
		}
	}
